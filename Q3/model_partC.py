...
class MNISTDiffusion(nn.Module):
    ...
    @torch.no_grad()
    def sampling(self, n_samples, labels, clipped_reverse_diffusion=True, device="cuda"):
        ...
        for i in tqdm(range(self.timesteps - 1, -1, -1), desc="Sampling"):
            ...
            if clipped_reverse_diffusion:
                x_t = self._reverse_diffusion_with_clip(x_t, t, labels, noise)
            else:
                x_t = self._reverse_diffusion(x_t, t, labels, noise)

        x_t = (x_t + 1.0) / 2.0
        return torch.clamp(x_t, 0.0, 1.0)

    @torch.no_grad()
    def ddim_sampling(self, n_samples, labels, ddim_steps=20, eta=0.0, device="cuda", clip_x0=True):
        x_t = torch.randn(
        ...
        for i in tqdm(range(len(step_indices)), desc="DDIM Sampling"):
            ...
            pred_noise = self.model(x_t, t_batch, labels)
            ...
            x_t = torch.sqrt(alpha_bar_prev) * x0_pred + direction + sigma * noise
    ...
    @torch.no_grad()
    def _reverse_diffusion(self, x_t, t, labels, noise):
    ...
    @torch.no_grad()
    def _reverse_diffusion_with_clip(self, x_t, t, labels, noise):
        pred = self.model(x_t, t, labels)
        ...
        if t.min() > 0:
            alpha_t_cumprod_prev = self.alphas_cumprod.gather(-1, t - 1).reshape(x_t.shape[0], 1, 1, 1)
            mean = (
                (beta_t * torch.sqrt(alpha_t_cumprod_prev) / (1.0 - alpha_t_cumprod)) * x_0_pred
