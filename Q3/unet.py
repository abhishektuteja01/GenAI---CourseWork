...
class ResidualDownsample(nn.Module):
    """
    ShuffleNetV2 unit for downsampling
    """
    def __init__(self, in_channels, out_channels):
        super().__init__()
        self.branch1 = nn.Sequential(
...
class TimeMLP(nn.Module):
    """
    Inject conditioning embedding into feature maps
    """
    ...
    def forward(self, x, emb):
        emb = self.mlp(emb).unsqueeze(-1).unsqueeze(-1)
        x = x + emb
        return self.act(x)
...
class EncoderBlock(nn.Module):
    ...
    def forward(self, x, emb=None):
        ...
        if emb is not None:
            x = self.time_mlp(x_shortcut, emb)
        else:
            x = x_shortcut
        x = self.conv1(x)
        return [x, x_shortcut]
...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x, x_shortcut, emb=None):
        x = self.upsample(x)
        x = torch.cat([x, x_shortcut], dim=1)
        x = self.conv0(x)
        if emb is not None:
            x = self.time_mlp(x, emb)
        x = self.conv1(x)
        return x
...
class Unet(nn.Module):
    """
    Simple conditional U-Net without attention
    """
    def __init__(
        self,
        timesteps,
    ...
    def forward(self, x, t=None, labels=None):
        ...
        if t is not None:
            ...
            if labels is not None:
                emb = emb + self.label_embedding(labels)

        encoder_shortcuts = []
        for encoder_block in self.encoder_blocks:
            x, x_shortcut = encoder_block(x, emb)
        ...
        for decoder_block, shortcut in zip(self.decoder_blocks, encoder_shortcuts):
            x = decoder_block(x, shortcut, emb)
...
if __name__ == "__main__":
    ...
    model = Unet(1000, 128, in_channels=1, out_channels=1, num_classes=10)
    y = model(x, t, labels)
    print(y.shape)